...
public class MappedRegistry<T> extends WritableRegistry<T> {
    ...
    public Holder<T> getOrCreateHolderOrThrow(ResourceKey<T> key) {
        return this.byKey.computeIfAbsent(key, (entry) -> {
            if (this.customHolderProvider != null) {
                throw new IllegalStateException("This registry can't create new holders without value");
            } else {
                this.validateWrite(entry);
                return Holder.Reference.createStandAlone(this, entry);
            }
        });
    }

    @Override
    public DataResult<Holder<T>> getOrCreateHolder(ResourceKey<T> key) {
        ...
        if (reference == null) {
            ...
            reference = Holder.Reference.createStandAlone(this, key);
    ...
    public Holder.Reference<T> createIntrusiveHolder(T value) {
        if (this.customHolderProvider == null) {
            ...
            return this.intrusiveHolderCache.computeIfAbsent(value, (valuex) -> {
                return Holder.Reference.createIntrusive(this, valuex);
            });
        } else {
