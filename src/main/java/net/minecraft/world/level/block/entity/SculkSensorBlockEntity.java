...
public class SculkSensorBlockEntity extends BlockEntity implements VibrationListener.VibrationListenerConfig {
    ...
    public SculkSensorBlockEntity(BlockPos pos, BlockState state) {
        ...
        this.listener = new VibrationListener(new BlockPositionSource(this.worldPosition), ((SculkSensorBlock)state.getBlock()).getListenerRange(), this, (VibrationListener.ReceivingEvent)null, 0.0F, 0);
    }
    ...
    public boolean shouldListen(ServerLevel world, GameEventListener listener, BlockPos pos, GameEvent event, @Nullable GameEvent.Context emitter) {
        return !this.isRemoved() && (!pos.equals(this.getBlockPos()) || event != GameEvent.BLOCK_DESTROY && event != GameEvent.BLOCK_PLACE) ? SculkSensorBlock.canActivate(this.getBlockState()) : false;
    }
