            ...
            if (movingobjectpositionblock.getType() == HitResult.Type.BLOCK) {
                ...
                if (!world.noCollision(entityboat, entityboat.getBoundingBox())) {
                    ...
                    if (!world.isClientSide) {
                        // CraftBukkit start
                        if (org.bukkit.craftbukkit.event.CraftEventFactory.callEntityPlaceEvent(world, movingobjectpositionblock.getBlockPos(), movingobjectpositionblock.getDirection(), user, entityboat).isCancelled()) {
                            return InteractionResultHolder.fail(itemstack);
                        }
