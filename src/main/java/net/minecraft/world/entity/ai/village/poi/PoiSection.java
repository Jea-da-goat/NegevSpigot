...
public class PoiSection {
    ...
    public void add(BlockPos pos, Holder<PoiType> holder) {
        if (this.add(new PoiRecord(pos, holder, this.setDirty))) {
            LOGGER.debug("Added POI of type {} @ {}", holder.unwrapKey().map((resourceKey) -> {
    ...
    private boolean add(PoiRecord poi) {
        ...
        this.byType.computeIfAbsent(holder, (holderx) -> {
