    ...
    private static class SilverfishMergeWithStoneGoal extends RandomStrollGoal {
        ...
        public void start() {
            if (!this.doMerge) {
                ...
                if (InfestedBlock.isCompatibleHostBlock(iblockdata)) {
                    // CraftBukkit start
                    if (org.bukkit.craftbukkit.event.CraftEventFactory.callEntityChangeBlockEvent(this.mob, blockposition, InfestedBlock.infestedStateByHost(iblockdata)).isCancelled()) {
                        return;
                    }
                    // CraftBukkit end
