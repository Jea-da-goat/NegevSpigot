...
public class Allay extends PathfinderMob implements InventoryCarrier {
    ...
    public Allay(EntityType<? extends Allay> type, Level world) {
        ...
        this.dynamicVibrationListener = new DynamicGameEventListener<>(new VibrationListener(entitypositionsource, 16, this.vibrationListenerConfig, (VibrationListener.ReceivingEvent) null, 0.0F, 0));
    ...
    public Iterable<BlockPos> iteratePathfindingStartNodeCandidatePositions() {
        ...
        int i = Mth.floor(axisalignedbb.minX - 0.5D);
        int j = Mth.floor(axisalignedbb.maxX + 0.5D);
        int k = Mth.floor(axisalignedbb.minZ - 0.5D);
        int l = Mth.floor(axisalignedbb.maxZ + 0.5D);
        int i1 = Mth.floor(axisalignedbb.minY - 0.5D);
        int j1 = Mth.floor(axisalignedbb.maxY + 0.5D);
    ...
    private class AllayVibrationListenerConfig implements VibrationListener.VibrationListenerConfig {
        ...
        public boolean shouldListen(ServerLevel world, GameEventListener listener, BlockPos pos, GameEvent event, GameEvent.Context emitter) {
            if (Allay.this.getLevel() == world && !Allay.this.isRemoved() && !Allay.this.isNoAi()) {
    ...
    private class JukeboxListener implements GameEventListener {
        ...
        public boolean handleGameEvent(ServerLevel world, GameEvent.Message event) {
            if (event.gameEvent() == GameEvent.JUKEBOX_PLAY) {
                Allay.this.setJukeboxPlaying(new BlockPos(event.source()), true);
                return true;
            } else if (event.gameEvent() == GameEvent.JUKEBOX_STOP_PLAY) {
                Allay.this.setJukeboxPlaying(new BlockPos(event.source()), false);
                return true;
            } else {
                return false;
            }
